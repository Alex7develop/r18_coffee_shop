...
//         'https://thingproxy.freeboard.io/fetch/https://dev.r18.coffee/api/mainpage/coffee'
//       );
...
//       const data = await response.json();
//       return transformCoffeeData(data);
//     } catch (error) {
...
//   const transformCoffeeData = (data) => {
...
//       const colors = ['brown']; // Предполагаем, что кофе имеет соответствующие цвета.

//       const transformedItem = {
//         article: item.ID,
//         title: item.NAME,
//         colors: colors.map(() => ({
//           name: '', // Добавьте соответствующие имена цветов, если нужно
//           value: ''
//         })),
//         price: `${uniqueOffersArray[0]?.PRICE} p.`,
//         composition: uniqueOffersArray
//           .map((offer) => offer.PROPERTY_OPISANIE_TEXT_VALUE.TEXT)
//           .filter((v, i, a) => a.indexOf(v) === i),
//         link: '#',
//         description: `${item.NAME} ${uniqueOffersArray[0]?.PROPERTY_SORT_VALUE || ''}`,
//       };
...
//             .map((offer) => offer.PROPERTY_Q_VALUE)
...
//   fetchCoffeeData().then((sliderCoffeeData) => {
//     if (sliderCoffeeData) {
//       const sliderCoffe1 = document.querySelector('.coffee__wr-slider-top');
//       if (sliderCoffe1) {
//         const filterList = sliderCoffe1.querySelector('.sl-prod__filter-list');
